...
/* Clients, for the strip under the hero. Adding one is adding an entry here
   ...
   change. Keep `width` and `height` as the file's real pixel size so the strip
   reserves the right space before the image loads.

   `tone` describes the artwork, not a preference: it is the colour the mark
   itself is drawn in. A dark mark cannot be seen on this page, so it is given a
   light plate to sit on; a reversed mark goes straight onto the background.
   Always ask a client for reversed artwork if they have it - it looks better
   than a plate, and then this is just `tone: "light"`. */
